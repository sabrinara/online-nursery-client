...
const ProductCard = ({ products }: { products: TProducts[] }) => {
  // Explicitly define the type for cart state
  ...
  const handleAddToCart = (product: TProducts) => {
    ...
    const existingItem = cartItems.find((item: TProducts) => item._id === product._id);

    if (existingItem) {
      if (existingItem.quantity < product.quantity) {
        const updatedCart = cartItems.map((item: TProducts) =>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            ...
            <div className="relative group bg-orange-50">
              <Link to={`/allproducts/${item._id}`}>
                <img
                  src={item.imageUrl}
                  alt={item.title}
                  className="w-full h-80 transform hover:scale-105 transition-transform duration-300"
                />
              </Link>
        ...
        <Link to={"/allproducts"}>
          <div className="flex items-center justify-center gap-4 my-20 md:my-0 md:mt-28 hover:animate-wobble">
            <button className="relative flex items-center justify-center border-2 border-green-800 text-green-900 px-6 py-3 rounded-full w-20 h-20 bg-white shadow-lg transition duration-300"></button>
